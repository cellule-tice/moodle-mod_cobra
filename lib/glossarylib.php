...
function cobra_export_myglossary($data) {
    ...
    $records = array();
    $records[0] = array(
        utf8_decode(get_string('entry', 'cobra')),
        utf8_decode(get_string('category')),
        'Autres formes',
        utf8_decode(get_string('translation', 'cobra')),
        'Texte source',
        utf8_decode('Cliqué dans ...'));
    $csvexport->add_data($records[0]);
    foreach ($data as $entry) {
        $record = array(
            utf8_decode($entry->entry),
            utf8_decode($entry->category),
            utf8_decode($entry->extrainfo),
            utf8_decode($entry->translations),
            utf8_decode($entry->sourcetexttitle),
